...
command: "crystal-htop-mem-bar.widget/widget_runner.sh"
...
update: (output, domEl) ->
  ...
  if not entries or entries.length < 6
    return  # Abort update if data is insufficient or incorrectly formatted
  ...
  if not rgbaRegex.test(fillColor) or not rgbaRegex.test(borderColor)
    fillColor = "rgba(255, 255, 255, 0)"  # Default color if invalid
    borderColor = "rgba(255, 255, 255, 0)"

  updateMemStat = (sel, rawValue, humanValue, color) ->
  ...
  # Update memory bars
  updateMemStat 'used', rawUsed, humanUsed, fillColor
  updateMemStat 'available', rawAvailable, humanAvailable, "rgba(255, 255, 255, 0)"
